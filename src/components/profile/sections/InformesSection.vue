        ...
        <div v-if="selectedReport" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="selectedReport = null">
          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg p-6 z-10">
            <div class="flex items-center justify-between mb-4">
              ...
              <button @click="selectedReport = null" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors font-bold text-xs uppercase">Cerrar</button>
            </div>
            <div class="space-y-3 text-sm text-gray-700 dark:text-gray-300">
              ...
              <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Notas personales</label>
                <textarea 
                  v-model="selectedReport.notes" 
                  rows="3" 
                  placeholder="Añade tus notas privadas aquí..."
                  class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 resize-none transition-colors"
                  @blur="saveReportNotes(selectedReport)"
                ></textarea>
                <p class="text-[10px] text-gray-400 mt-1">Se guardan automáticamente al salir del campo.</p>
              </div>
            </div>
...
const shareReport = (report) => {
  showToast(`Compartiendo informe "${report.title}" con especialista...`, 'info');
};

const saveReportNotes = (report) => {
  // TODO: reemplazar con llamada a endpoint
  showToast('Notas privadas guardadas correctamente', 'info');
};
