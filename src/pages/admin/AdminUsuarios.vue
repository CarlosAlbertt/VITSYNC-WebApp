...
// ── Helpers ───────────────────────────────────────────────────────────────────
const fullName = (u) => [u.name, u.firstName, u.secondName].filter(Boolean).join(' ');

const roleBadgeClass = (role) => {
    switch (role) {
        case 'ADMIN': return 'bg-purple-100 text-purple-800';
        case 'MEDICO': return 'bg-teal-100 text-teal-800';
...
const roleColors = (role) => {
    switch (role) {
        ...
        case 'MEDICO': return 'bg-teal-100 text-teal-700';
        case 'PACIENTE': return 'bg-blue-100 text-blue-700';
...
const saveUser = async () => {
    ...
    try {
        ...
        formError.value = msg || 'Error al guardar el usuario.';
    } finally {
...
const confirmDelete = async () => {
    ...
    try {
        ...
        showToast(`"${fullName(userToDelete.value)}" eliminado correctamente.`);
        closeDeleteModal();
        await loadData();
    } catch (err) {
